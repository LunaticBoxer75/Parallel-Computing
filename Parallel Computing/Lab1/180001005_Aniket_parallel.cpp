...
string check_red(string p){	//Iterates through complete grammar to check if the string p can be reduced or not
	...
	#pragma omp parallel num_threads(size)
	{
		...
		#pragma omp for    	// Parallelizes the next loop
		for(j=0;j<no_prod;j++){
		...
		#pragma omp critical	// To prevent concurrent writes
		curr1=join(curr1,curr);
	}	
	return curr1;
}
		...
		for(j=k;j<str.size();j++){
			...
			#pragma omp parallel num_threads(size)
			{
				...
				for(l=j-k;l<j;l++){
					...
					string temp1 = get_all_comb(parse_matrix[j-k][l],parse_matrix[l+1][j]);
